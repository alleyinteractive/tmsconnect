...
function tmsc_add_type_taxonomies( $post_type, $taxonomy, $edit_slug, $labels ) {
	if ( 'exhibition' === $post_type ) {
		$tag_url = add_query_arg( array( 'taxonomy' => 'exhibition-types', 'post_type' => 'exhibition' ), 'edit-tags.php' );
		add_submenu_page(
			$edit_slug,
...
function tmsc_highlight_edit_tag() {
	...
	if ( 'exhibition' === $typenow && 'exhibition-types' === $taxnow ) {
		$parent_file = add_query_arg( array( 'post_type' => 'exhibition', 'taxonomy' => 'exhibitions' ), 'edit.php' );
		$submenu_file = add_query_arg( array( 'taxonomy' => 'exhibition-types', 'post_type' => 'exhibition' ), 'edit-tags.php' );
	}
}
